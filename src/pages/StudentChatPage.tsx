    ...
    <div className="chat-page-shell h-[100dvh] flex flex-col overflow-hidden">
      <header className="chat-page-header shrink-0"><div className="chat-page-header-inner mx-auto max-w-6xl px-3 sm:px-5"><div className="flex h-12 items-center justify-between sm:h-14">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="min-w-0"><h1 className="chat-page-title truncate text-sm font-bold">Career adviser</h1><p className="chat-online-label text-xs font-medium uppercase tracking-wider">Profile-aware guidance</p></div>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">{userContext.currentGrade && <Badge variant="secondary" className="chat-level-badge hidden px-2 py-0.5 text-xs sm:inline-flex">{userContext.currentGrade}</Badge>}<Button size="sm" variant="ghost" onClick={startNewChat} disabled={!isInitialized || isLoading} className="chat-reset h-8 px-2 text-xs sm:px-3"><Plus className="mr-1 h-3.5 w-3.5" />New chat</Button><ThemeToggle /></div>
      </div></div></header>
      ...
      <div className="relative flex min-h-0 flex-1 overflow-hidden">
        ...
        <aside className={`absolute inset-y-0 left-0 z-20 flex w-[min(18rem,86vw)] shrink-0 flex-col border-r border-slate-200 bg-white transition-transform duration-200 lg:relative lg:w-72 ${isHistoryOpen ? 'translate-x-0' : '-translate-x-full lg:-ml-72'}`}>
          <div className="flex items-center justify-between border-b border-slate-100 px-3 py-3"><div><p className="text-xs font-bold text-slate-800">Chat history</p><p className="mt-0.5 text-xs text-slate-500">Your conversations stay on this device.</p></div><Button variant="ghost" size="icon" onClick={() => setIsHistoryOpen(false)} className="h-7 w-7 lg:hidden"><X className="h-4 w-4" /></Button></div>
        ...
        <main className="flex min-w-0 flex-1 flex-col bg-[#f8fafc]"><div className="min-h-0 flex-1 overflow-hidden"><ScrollArea className="h-full"><div className="chat-message-list mx-auto max-w-3xl space-y-5 px-3 py-5 sm:px-5 sm:py-7">
          ...
          <div className="chat-composer-wrap shrink-0 border-t border-slate-200 bg-white p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4"><div className="mx-auto max-w-3xl">
            ...
            <ChatInput
              ...
              placeholder="Ask about your subjects, grades or next steps…"
              className="chat-composer"
              inputClassName="chat-composer-input"
              buttonClassName="chat-send-button"
              aria-label="Ask the career adviser"
              autoFocus
            />
            <div className="mt-2 flex items-center justify-between px-1"><p className="chat-composer-note flex items-center text-xs"><Sparkles className="mr-1 h-3 w-3" />Uses your saved profile and grade data</p>{userContext.hasRecordedGrades ? <span className="text-xs font-medium text-emerald-700">Grades included</span> : <span className="text-xs text-slate-500">No grades uploaded yet</span>}</div>
          </div></div>
        </main>
      </div>
    </div>
